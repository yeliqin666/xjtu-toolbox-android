...
class DzpzLogin(
    ...
    override fun postLogin(response: Response) {
        ...
        userId = findLoginIdWeaver(response)
        if (userId != null) {
        ...
        Log.d(TAG, "postLogin: re-accessing ecology pages")
        val warmupUrls = listOf(
            "$BASE_URL/wui/index.html",
            "$BASE_URL/spa/workflow/static4form/index.html",
            "$BASE_URL/api/ecode/sync"
        )
        for (url in warmupUrls) {
            try {
                val req = Request.Builder()
                    .url(url)
                    .header("Referer", "$BASE_URL/login/Login.jsp")
                    .get()
                    .build()
                val resp = client.newCall(req).execute()
                val final = resp.request.url.toString()
                val bodyHead = try { resp.peekBody(400).string() } catch (_: Exception) { "" }
                userId = findLoginIdWeaver(resp)
                resp.close()
                Log.d(TAG, "postLogin: warmup finalUrl=$final, userId=$userId")
                Log.d(TAG, "postLogin: warmup bodyHead=${bodyHead.take(240)}")
                if (userId != null) break
            } catch (e: Exception) {
                Log.w(TAG, "postLogin: warmup failed url=$url msg=${e.message}")
            }
        }
    ...
    fun reAuthenticate(): Boolean = synchronized(reAuthLock) {
        try {
            ...
            if (com.xjtu.toolbox.util.WebVpnUtil.isAtTargetSite(ssoFinalUrl, "dzpz.xjtu.edu.cn")) {
                userId = findLoginIdWeaver(ssoResp)
                lastUserIdFromPostLogin = userId
