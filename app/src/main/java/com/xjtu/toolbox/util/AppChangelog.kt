...
object AppChangelog {
    ...
    val ENTRIES: List<Pair<String, VersionChangelog>> = listOf(
        "4.9.8" to VersionChangelog(
            items = listOf(
                "🧪" to "预览版：新版考勤对齐（学期码修复、直连登录、考勤课表源、打卡流水）",
                "🧹" to "移除课程回放（class 平台）；思源学堂的回放播放与下载不受影响",
                "🎮" to "新增「课余」分类：合成西交大、GPA 2048、五子棋、围棋、象棋",
                "🫱" to "匹配交友回归：修好了「怎么选都不出码」——失败不再伪装成加载中，扫码取景提到 1080p 并开启强识别",
                "🫧" to "手机底栏默认换成可以拖动的玻璃胶囊；想换回来：设置 → 外观 → 界面风格 → 经典（经典更省电，所有玻璃效果一起关掉）",
                "🤖" to "屁岱皮肤里的「纸色」改成真正镂空：眼睛等处直接透出后面的底栏，玻璃底栏上也不会露出一块底色",
                "🪟" to "首页、学辅、我的、屁岱和各功能页的顶栏都换成玻璃，标签和筛选条并进顶栏，内容从下面滚过；去掉顶栏下沿那道灰影，模糊加重，接缝处不再发清",
                "📱" to "手机固定竖屏；手机和平板按屏幕最短边 600dp 区分，手机横过来不再变成平板排布",
                "👆" to "带分段标签的页面都能左右滑动切换；往下拉先展开大标题、再算下拉刷新，提示文字改成中文",
                "📐" to "平板横屏适配二十来个页面：卡片分两三列，教师主页、匹配交友、校历、空闲教室、图书馆、体测等改成左右两栏",
                "📅" to "日程页改版：今日 / 日程 / 学期三栏，周选择器、「回本周」、已考完的考试折叠；换课表来源后不用重启就能切学期",
                "📚" to "教材中心、书架、全文库合并成一个入口；修复全文库没地方输关键词",
                "💳" to "校园卡余额卡改浅色、时间范围收进右上角菜单；充值后回首页余额立即更新；月度趋势不再显示「¥-0」",
                "📶" to "修复连着校园网却显示「校外 · WebVPN」：原来的探测点（旧考勤）已停用，改为同时探测几台内网服务",
                "🔗" to "WebVPN 打开转换后的网址直接进目标页，不再停在网关的登录前页",
                "🪑" to "图书馆：整页一起滚动；移除座位地图和座位推荐",
                "🔕" to "主动提醒选「关」后，点屁岱也不再冒气泡",
                "⚙️" to "设置页重新分组：外观、通用、功能、通知与提醒、数据与隐私……",
                "🚀" to "动画和滚动更流畅：渐变背景改由 GPU 绘制，几处常驻动画不再每帧重组；平板侧栏展开收起时藏起来的页面不再跟着重排；首页点开功能的放大转场更快",
                "🗓️" to "快速考勤流水换成最近两周的日期条；各处选日期改用系统月历",
                "🧑‍🏫" to "教师主页：卡片信息改成两行小字，筛选改成一整行条件栏",
                "🫂" to "匹配交友按真正的当前学期出码，可在分享设置里选历史学期；自己添加的日程不会写进码里",
                "📄" to "学辅资料预览改成底部弹窗；浏览器可以分享网页标题和链接",
                "↩️" to "平板分栏页（日程、教师主页、学辅）按返回直接退出，不再先清右栏",
                "🩹" to "修复启动时教务系统报错导致的闪退；通知卡片左下角标签不再被截断；空闲教室、成绩卡片展开时不再换列",
                "🤖" to "屁岱：精简系统提示词和工具说明，工具报错改成统一的结构化字段，回答更直接",
            )
        ),
