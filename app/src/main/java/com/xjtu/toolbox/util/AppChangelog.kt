...
object AppChangelog {
    ...
    val ENTRIES: List<Pair<String, VersionChangelog>> = listOf(
        "4.10.0" to VersionChangelog(
            items = listOf(
                "🎨" to "底栏的屁岱可以换样子和颜色了：八种形状、十几种配色，在屁岱页右上角「配置」最下面挑，选完当场就变，重启也记得",
                "🌗" to "形象颜色多一项「跟随主题」并默认选中，深浅色模式下都看得清，不会挑了个颜色反而看不见",
                "⚡" to "点开盒子更快了，从点图标到看见首页的等待缩短了",
                "💬" to "屁岱回答长内容时顺了很多，长对话里不再一顿一顿地往外蹦字",
                "🧾" to "通知公告、考勤流水这类长列表滑起来更跟手，下拉刷新也不再整屏闪一下",
                "🔋" to "顺手清掉几处会让页面小幅卡顿的小毛病，日常用着更省电一些"
            )
        ),
