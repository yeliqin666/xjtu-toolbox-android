    ...
    val ENTRIES: List<Pair<String, VersionChangelog>> = listOf(
        "4.9.8" to VersionChangelog(
            items = listOf(
                ...
                "📅" to "日程页改版：今日 / 周视图 / 学期三栏；周视图底部悬浮一颗「‹ 第 N 周 ›」玻璃胶囊翻周，点它选周（含全学期总览和「回本周」），滚动时自动让开；已考完的考试折叠；换课表来源后不用重启就能切学期",
                ...
                "✨" to "屁岱首屏重做：按时段打招呼，问题卡片直接带上下节课、最近考试、饭卡余额的预览，点一下就问；「我还能」标签一键填入示范问题；没配模型时换成引导卡",
                "🧰" to "屁岱工具合并为 26 个：能往本学期课表里加日程（会查时间冲突）、回答里能显示公式和图片；名字、记忆等设置从下一个新对话起生效；拉取模型列表改成输入框右侧的图标；去掉能力开关和工具次数上限",
                "💬" to "屁岱闲话换了一批：删掉无聊和容易冒犯的，加上按你课表说的关心（比如体育课提醒带鞋、明早第一节有课）和更多小性格",
                "🎮" to "课余大改：合成西交大修好落点跟手偏移，校徽做成立体棋子，加了瞄准线和合成特效；GPA 2048 改用西交大 4.3 制（F 到 A+），有滑动合成动画、每局 3 次撤销和盘面绩点；五子棋、围棋、象棋换上木纹棋盘和立体棋子",
                "🩹" to "修复围棋落子后看不到棋子、五子棋人机对战偶发闪退、围棋和象棋的联机大厅被顶栏挡住一半",
                "🫧" to "设置里切换界面风格（玻璃 / 经典）立即生效，不用重启",
                "📚" to "学辅列表改版：文件夹排成两列卡片，文件合成一张分组列表",
                "🧭" to "空闲教室选校区一屏排满，不用横滑；思源学堂进入课程时不再闪出首页背景",
                "🚀" to "首页头图的流动渐变只在打开时动几秒，滑动更顺；报错提示条贴回底栏上方",
                "🗳️" to "版本评价弹窗只留「有问题」「挺好」两个按钮",
                "💭" to "在屁岱页点屁岱不再从底栏冒气泡挡住输入框：首屏时由屁岱自己换句话说，聊天中在输入框上方冒一句小话，几秒后收起",
                "🧩" to "屁岱配置：拉取模型列表后，选中项和输入框保持一致，第一项是「默认」",
                "📊" to "成绩报表修复小学期的课被并进春季学期；暑假重修、补考的成绩单独成组",
                "🎮" to "修复合成西交大打开后一片空白",
                "📅" to "修复节假日那周自己加的日程（包括让屁岱加的）刚出现就消失：放假只停教务的课；屁岱加的日程写进真正的本学期，日程页、首页提醒、桌面小组件立即显示；导出日历按日程的实际时间",
                "🔌" to "联机对战修好连不上：连上后不再立刻断开，局域网和蓝牙谁先通用谁，二维码里不再误填流量或 VPN 的地址，第一次开房也能走蓝牙，离开页面自动断开",
                "🏠" to "首页合成一张「今日」卡：问候、网络与子系统状态、下一项安排、余额 / 今日消费 / 考试倒计时、常用入口都在里面；下面的分类卡不再重复这些信息；卡片与彩虹两种风格都重新设计",
                "🤖" to "屁岱回答下面的数据卡片重做：课表按钟点排、考试有日期块和倒计时、空教室带全天节次条、成绩突出 GPA、校园卡做成卡面；跳转按钮更醒目",
                "🕹️" to "小游戏入口重做：两款休闲游戏做成大卡、最高分放大，棋类一行一个，标出支持的对局方式",
                "✨" to "全 App 加上克制的动效：页面打开时各块依次登场，余额、GPA、体测总分等数字滚动出来，条形图从左长出；正在上的课显示已上进度，考试前一天倒计时图标会轻晃；列表首次加载显示骨架而不是转圈",
                "🚀" to "修复首页和各功能页顶栏折叠 / 展开时掉帧，以及平板侧栏展开收起时卡顿",
                "🧹" to "清理七十多处不再使用的旧代码",
            )
        ),
