...
fun SettingsScreen(
    ...
    // 外观设置写进 CredentialStore 即可：主题、底栏、首页都订阅 AppearanceSettings，自动跟着变
    var theme by remember { mutableStateOf(credentialStore.homeTheme) }
...
/**
 * 从 [com.xjtu.toolbox.update.AppChangelog] 派生：
 * 设置页 → 关于 → 更新日志 与启动弹窗共享同一份数据，无需重复维护。
 */
private fun changelogItems(): List<ChangelogEntry> =
    com.xjtu.toolbox.update.AppChangelog.ENTRIES.map { (version, log) ->
        ChangelogEntry(
            version = "v$version",
